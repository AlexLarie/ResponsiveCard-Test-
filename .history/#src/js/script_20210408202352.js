...
let icons = document.querySelectorAll('.nav__img--wrapper');
...
for (let i=0; i<icons.length; i++){
   icons[i].addEventListener('mouseleave', (e)=>{
      e.target.firstElementChild.firstElementChild.setAttribute('fill', 'gray')
   })
}

let popapShow = document.querySelector('#popap__button');
let popap = document.querySelector('.popap');

popapShow.addEventListener('click', ()=>{
   popap.classList.toggle('active')
})

let burger = document.querySelector('.burger__right');
let burgerButton = document.querySelector('.burger__button');
let burgerMenu = document.querySelector('.nav--burger');
let burgerRed = document.querySelector('.burger__right');

burger.addEventListener('click', ()=>{
   burgerButton.classList.toggle('active')
   burgerMenu.classList.toggle('active')
   burgerRed.classList.toggle('active')
})
// Пикер для меню
